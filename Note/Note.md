...
## 控制流
rust支持与C/C++中类似的if else控制流语句，与C++有点不同的是，rust 中的if else可以如下面这样写来进行返回值返回
```rust
{
    if a.age == b.age {
        a.name.cmp(&b.name)
...
同时也有类似于C/C++中的switch的match语法，比如
```rust
fn main() {
    let number = 3;
    ...
    match number {
        ...
        2 => println!("是 2"),
        3 => println!("是 3"),
        _ => println!("是其他数字"), // _ 代表 default，匹配剩余所有情况
    }
}
```
相当于C++中的
...
int main(){
	...
	switch(number){
		case 1:
			std::cout << "是 1" << std::endl;
			break;
		case 2:
			std::cout << "是 2" << std::endl;
			break;
		case 3:
			std::cout << "是 3" << std::endl;
			break;
		default:
			std::cout << "是其他数字" << std::endl;
			break;
		
	}
}
```
同时match语法一定程度上也可以替代if else结构，让if else控制流结构更加清晰
```rust
